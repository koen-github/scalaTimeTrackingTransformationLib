...
{  //here comes your translation!
  ...
  val duration  = ttdSource.task.map{x =>
    ...
    val thing: List[TimeIntervalAbstract] = x.intervals.map{
      ...
      if(x.duration.toDouble/ttdTarget.granularity.toDouble != 1.0)
       {

         val newDuration = Math.round(x.duration/ttdTarget.granularity.toDouble)*ttdTarget.granularity
         //val snapMinutes = Math.round(new Date(x.start).getMinutes/ttdTarget.snapInterval)*10
         val endie = Math.round(x.end-(ttdTarget.granularity.toDouble/2))
